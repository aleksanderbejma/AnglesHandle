...
function GitHubDataController($scope, $http, $element){
	...
	var loadRepos = function (repos_url) {
		$http.get(repos_url+'?per_page=100&access_token='+$scope.gitAccessToken, {headers: {'Content-type': 'application/json'}})
		.success(function (data) {
			$scope.repoData = data;
	...
	var loadRepoContributors = function (cont_url) {
		var i = 0;
		$http.get(cont_url+'?per_page=100&access_token='+$scope.gitAccessToken, {headers: {'Content-type': 'application/json'}})
	...
	var loadUserInfo = function (cont_login) {
		...
		$scope.loaded = false;
		$http.get("https://api.github.com/users/"+cont_login+'?access_token='+$scope.gitAccessToken, {headers: {'Content-type': 'application/json'}})
		.success(function (data) {
	...
	$scope.showUserInfo = function (login) {
		$http.get("https://api.github.com/users/"+login+"/repos?per_page=100&access_token="+$scope.gitAccessToken, {headers: {'Content-type': 'application/json'}})
		.success(function (data) {
			$scope.userRepos = data;
			$scope.reposGeted = data.length > 0;
			$http.get("https://api.github.com/users/"+login+"?access_token="+$scope.gitAccessToken, {headers: {'Content-type': 'application/json'}})
			.success(function (data) {
				$scope.userInfo = data;	
	...
	$scope.showRepoInfo = function (url) {
		$http.get(url+"?access_token="+$scope.gitAccessToken, {headers: {'Content-type': 'application/json'}})
		.success(function (data) {
			$scope.repoInfo = data;
	...
	$scope.showAngularRepositories = function () {
		$http.get("https://api.github.com/orgs/angular/repos?per_page=100&access_token="+$scope.gitAccessToken, {headers: {'Content-type': 'application/json'}})
		.success(function (data) {
